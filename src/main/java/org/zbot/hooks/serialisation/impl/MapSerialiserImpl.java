 ...
 */
public class MapSerialiserImpl implements IMapSerialiser {
	...
	public void serialise(HookMap map, OutputStream os) throws IOException {
		...
		List<ClassHook> classes = map.getClasses();
		...
		for(ClassHook c : classes) {
			...
			for(MethodHook m : methods) {
				...
				writeCode(contentDos, m.getInstructions(), pool);
			}
		}
		
		DataOutputStream dos = new DataOutputStream(os);
		dos.writeInt(map.getVersion());
		pool.write(dos);
	...
	private void writeObfRefType(DataOutputStream dos, Pool pool, ObfuscatedData d) throws IOException {
		dos.writeInt(pool.allocateString(d.getObfuscated()));	
		dos.writeInt(pool.allocateString(d.getRefactored()));
	}
