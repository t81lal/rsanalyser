...
public class CallVisitor extends Visitor {
	...
	private void search(Map<String, ClassStructure> tree, MethodNode vertex) {
		if (tree.get(vertex.owner.name).callGraph.containsVertex(vertex))
			return;
		tree.get(vertex.owner.name).callGraph.addVertex(vertex);
		used++;
		aParnyLoop: for (AbstractInsnNode ain : vertex.instructions.toArray()) {
			if (ain instanceof MethodInsnNode) {
				...
				if (tree.containsKey(min.owner)) {
					...
					if (edge != null) {
						tree.get(vertex.owner.name).callGraph.addEdge(vertex, edge); // method is called, graph it
					...
					for (ClassStructure supertype : cs.supers) { // do the same for all supertypes and superinterfaces
						...
						if (superedge != null) {
							tree.get(vertex.owner.name).callGraph.addEdge(vertex, superedge);
