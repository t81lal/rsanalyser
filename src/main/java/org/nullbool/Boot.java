...
import org.nullbool.impl.r105.AnalysisProvider105Impl;
...
public class Boot {
    ...
    public static final File gamepacksDirectory = Paths.get(System.getProperty("user.home"), "Dropbox", "OSRS RE collection", "gamepacks").toFile();

    public static void main(String[] args) throws Exception {
        System.out.printf("Remote rev: %d.%n", RSVersionHelper.getVersion(RSVersionHelper.getServerAddress(58), 77, 100));
        File gamepackFile = new File(gamepacksDirectory, args[0] + ".jar");
        bootstrap();
    ...
    private static void bootstrap() throws Exception {
        ...
        AnalysisProviderRegistry.register(new RegistryEntry(new ProviderCreator() {
            @Override
            public AbstractAnalysisProvider create(Revision rev) throws Exception {
                return new AnalysisProvider104Impl(rev);
            }
        }).addFilter(new Filter<Revision>() {
            @Override
            public boolean accept(Revision t) {
        ...
        AnalysisProviderRegistry.register(new RegistryEntry(new ProviderCreator() {
            @Override
            public AbstractAnalysisProvider create(Revision rev) throws Exception {
                return new AnalysisProvider105Impl(rev);
            }
        }).addFilter(new Filter<Revision>() {
            @Override
            public boolean accept(Revision t) {
