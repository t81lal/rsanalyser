...
import org.zbot.topdank.eventbus.util.ReflectionHelper.Filter;
...
public class ClassStructure extends ClassNode {
	...
	public String toString() {
		return name;
		/* final StringBuilder sb = new StringBuilder(name).append(" extends ").append(superName); if (interfaces.size() == 0) return sb.toString();
		 * sb.append(" implements ").append(interfaces.get(0)); for (final String iface : interfaces.subList(1, interfaces.size())) {
	...
	public List<MethodNode> getMethods(Filter<MethodNode> filter) {
